        ...
        methods: {
            ...
            read(f) {
                ...
                rd.onload = e => {  //this.readAsArrayBuffer函数内，会回调this.onload函数。在这里处理结果
                    ...
                    this.nameList = sheets[this.sheetName[0]];
                    this.nameList.splice(0,4);

                    var namelists = [{'nameLists' : this.nameList}];
                    console.log(namelists);
                    var chickList = namelists;
                    this.postRequest('/chick/daychick',chickList).then(resp => {
